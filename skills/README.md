...
clawhub skill publish "$(pwd)/skills/google-messages" \
  ...
  --owner fdsouvenir \
  --version 0.3.3 \
  --changelog "Use evidence-based doctor health states, remove legacy sync-timestamp heuristics, and document archive upgrade behavior." \
...
```sh
clawhub inspect google-messages-local-archive --version 0.3.3 --files
```
...
```sh
tmpdir="$(mktemp -d)"
clawhub --workdir "$tmpdir" install google-messages-local-archive --version 0.3.3
